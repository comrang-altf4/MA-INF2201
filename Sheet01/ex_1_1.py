...
def get_mean_val_own(img, rand_coords = [[0, 0]], size = None, verbose = True):
    ...
    # Get integral image
    img_integral = get_integral_img_own(img)
...
def get_integral_img_own(img):
    ...
    for i in range(1, img.shape[0]):
        for j in range(1, img.shape[1]):
            img_integral[i, j] = img_integral[i - 1, j] + img_integral[i, j - 1] - img_integral[i - 1, j - 1] + img[i, j]
    return img_integral
...
def main():
    ...
    # Task 3.1a
    img_integral_own = get_integral_img_own(img)
